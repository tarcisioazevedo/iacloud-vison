...
// ────────────────────────────────────────────────────────────────────────────
// Acordeão de site pra CLIENTE_* — header colapsável + cameras lazy-loaded.
// Reusa /cameras?siteId=X (já scopado por tenant via JWT no backend).
// ────────────────────────────────────────────────────────────────────────────
function ClienteSiteAccordionRow({
  site, onCameraClick,
}: {
  site: SiteRow
  ...
  const [expanded, setExpanded] = useState(false)
  const [liveOpenIds, setLiveOpenIds] = useState<Set<string>>(new Set())
  // Boxes começam expandidos por default — tenant médio tem 1-2 boxes por
  // site, manter aberto evita um clique extra no caso comum.
  const [boxesExpanded, setBoxesExpanded] = useState<Set<string>>(new Set())
  const [boxesInitialized, setBoxesInitialized] = useState(false)

  // Lazy: só busca quando o site abre. Cameras + Boxes em paralelo.
  const { data: camData,  isLoading: camLoading,  error: camError  } = useCameras(expanded ? { siteId: site.id } : undefined)
  const { data: edgeData, isLoading: edgeLoading, error: edgeError } = useEdgeNodes(expanded ? { siteId: site.id, includeOffline: true } : undefined)

  const cameras: Array<{ id: string; name: string; status?: string; active?: boolean; edgeNodeId?: string | null }> =
    (camData?.cameras ?? camData?.items ?? []) as Array<{ id: string; name: string; status?: string; active?: boolean; edgeNodeId?: string | null }>
  const edgeNodes = edgeData?.edgeNodes ?? []

  const isLoading = camLoading || edgeLoading
  const error     = camError ?? edgeError

  // Auto-expande todos os boxes na primeira carga
  useEffect(() => {
    if (expanded && !boxesInitialized && edgeNodes.length > 0) {
      setBoxesExpanded(new Set(edgeNodes.map(b => b.id)))
      setBoxesInitialized(true)
    }
  }, [expanded, edgeNodes, boxesInitialized])

  // Câmeras agrupadas por edgeNodeId; null/undefined = cloud-direct (sem box)
  const camerasByBox = useMemo(() => {
    const map = new Map<string, typeof cameras>()
    const cloudDirect: typeof cameras = []
    for (const cam of cameras) {
      if (cam.edgeNodeId) {
        const arr = map.get(cam.edgeNodeId) ?? []
        arr.push(cam)
        map.set(cam.edgeNodeId, arr)
      } else {
        cloudDirect.push(cam)
      }
    }
    return { byBox: map, cloudDirect }
  }, [cameras])
  ...
  return (
    <div className="rounded-lg border border-slate-200 dark:border-white/10 overflow-hidden bg-white/50 dark:bg-white/[0.02]">
      <button
        type="button"
        onClick={() => setExpanded(v => !v)}
        className="w-full flex items-center gap-3 px-3 py-2.5 hover:bg-slate-50 dark:hover:bg-white/[0.04] transition text-left"
      >
        {expanded
          ? <ChevronDown className="w-4 h-4 text-slate-400 shrink-0" />
          : <ChevronRight className="w-4 h-4 text-slate-400 shrink-0" />}
        <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-emerald-500/30 to-cyan-500/30 border border-emerald-500/30 flex items-center justify-center text-xs font-bold text-emerald-700 dark:text-emerald-200 shrink-0">
          {site.name[0]?.toUpperCase() ?? 'S'}
        </div>
        <div className="flex-1 min-w-0">
          <div className="text-sm font-medium text-slate-900 dark:text-white truncate">
          ...
          <div className="text-[11px] text-slate-500 mt-0.5 flex items-center gap-2">
            ...
            {site.address ? <span className="truncate max-w-[280px]">· {site.address}</span> : null}
          </div>
        </div>
        <span className="px-2 py-0.5 rounded text-[10px] bg-cyan-500/15 text-cyan-700 dark:text-cyan-300 border border-cyan-500/30 font-mono shrink-0">
          {site._count.cameras} {site._count.cameras === 1 ? 'câmera' : 'câmeras'}
        </span>
        {site.active ? (
          <span className="inline-flex items-center gap-1 text-[10px] text-emerald-600 dark:text-emerald-300 shrink-0">
            <CheckCircle2 className="w-3 h-3" /> Ativo
          </span>
        ) : (
          <span className="text-[10px] text-slate-500 shrink-0">Inativo</span>
        )}
      </button>
      ...
      {expanded && (
        <div className="border-t border-slate-200 dark:border-white/10 bg-slate-50/50 dark:bg-white/[0.02]">
          ...
          {error && !isLoading && (
            <div className="flex items-start gap-2 px-4 py-3 text-xs text-rose-500">
              <AlertTriangle className="w-3.5 h-3.5 shrink-0 mt-0.5" />
              <span>{formatApiError(error)}</span>
            </div>
          )}
          {!isLoading && !error && cameras.length === 0 && edgeNodes.length === 0 && (
            <div className="px-4 py-6 text-center text-xs text-slate-500">
              Nenhuma box ou câmera instalada neste site.
            </div>
          )}

          {/* CENÁRIO 1: site SEM box (residencial / cloud-direct puro) — render
              flat de câmeras, sem header técnico. UX limpa pra casa, condomínio
              pequeno, etc. */}
          {!isLoading && !error && edgeNodes.length === 0 && cameras.length > 0 && (
            <ul className="divide-y divide-slate-200 dark:divide-white/5">
              {cameras.map(cam => renderCameraRow(cam, liveOpenIds, toggleLive, onCameraClick))}
            </ul>
          )}

          {/* CENÁRIO 2/3: site COM box(es) — render box-by-box. Câmeras
              cloud-direct (sem box) só ganham header próprio quando coexistem
              com boxes (caso misto). */}
          {!isLoading && !error && edgeNodes.length > 0 && edgeNodes.map(box => {
            const boxOpen   = boxesExpanded.has(box.id)
            const boxCams   = camerasByBox.byBox.get(box.id) ?? []
            const boxOnline = box.status === 'ONLINE'
            return (
              <div key={box.id} className="border-b border-slate-200 dark:border-white/5 last:border-b-0">
                <button
                  ...
                  className="w-full flex items-center gap-3 px-3 py-2 hover:bg-slate-100 dark:hover:bg-white/[0.04] transition text-left"
                >
                  {boxOpen
                    ? <ChevronDown className="w-4 h-4 text-slate-400 shrink-0" />
                    : <ChevronRight className="w-4 h-4 text-slate-400 shrink-0" />}
                ...
                {boxOpen && (
                  <div className="bg-slate-50 dark:bg-black/20 border-t border-slate-200 dark:border-white/5">
                    {boxCams.length === 0 ? (
                      <div className="px-10 py-3 text-[11px] text-slate-500">
                        Nenhuma câmera vinculada a esta box.
                      </div>
                    ) : (
                      <ul className="divide-y divide-slate-200 dark:divide-white/5">
                        {boxCams.map(cam => renderCameraRow(cam, liveOpenIds, toggleLive, onCameraClick))}
                      </ul>
                    )}
                  </div>
                )}
          ...
          {/* Cloud-Direct cameras — só renderiza no caso misto (há boxes E há
              câmeras sem box). Caso "site sem box" já foi tratado no Cenário 1
              acima, sem header técnico. */}
          {!isLoading && !error && edgeNodes.length > 0 && camerasByBox.cloudDirect.length > 0 && (
            <div className="border-t border-slate-200 dark:border-white/5">
              <div className="px-3 py-2 text-[10px] uppercase tracking-wider font-bold text-slate-500 bg-slate-100 dark:bg-white/[0.02]">
                ☁️ Câmeras Cloud-Direct (sem edge box)
              </div>
              <ul className="divide-y divide-slate-200 dark:divide-white/5">
                {camerasByBox.cloudDirect.map(cam => renderCameraRow(cam, liveOpenIds, toggleLive, onCameraClick))}
              </ul>
            </div>
          )}
        </div>
