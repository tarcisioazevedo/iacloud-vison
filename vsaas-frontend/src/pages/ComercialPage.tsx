...
export function ComercialPage() {
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex items-start justify-between gap-3">
          ...
          {canConfig && (
            <Link to="/admin/comercial/config"
              className="shrink-0 flex items-center gap-1.5 px-3 py-2 rounded-lg bg-slate-50 dark:bg-white/5 hover:bg-slate-100 dark:bg-white/10 border border-slate-200 dark:border-white/10 text-xs text-slate-600 dark:text-slate-300 hover:text-white transition"
      ...
      <GlassCard className="p-2 border-slate-300 dark:border-slate-700/50">
                ...
                <button onClick={() => changeTab(tab.id)}
                  className={cn(
                    ...
                    isActive
                      ...
                      // Tabs INATIVAS: text-slate-600 dark:text-slate-300 (legível no fundo dark) + hover claro
                      : 'text-slate-600 dark:text-slate-300 hover:text-white hover:bg-slate-100 dark:bg-slate-800/50 border-transparent',
                  )}>
