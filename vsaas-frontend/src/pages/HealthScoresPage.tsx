    ...
    <div className="space-y-4">
      ...
      <GlassCard className="p-5 bg-gradient-to-br from-emerald-500/10 via-cyan-500/5 to-transparent border-emerald-500/20">
        <div className="flex items-start justify-between gap-3">
          ...
          <button
            onClick={() => mutate()}
            className="inline-flex items-center gap-1 px-3 py-2 rounded-lg border border-slate-300 dark:border-white/10 text-xs text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-50 dark:bg-white/5"
          >
...
function ScoreRow({ score, onSelect }: { score: ClienteHealthScore; onSelect: () => void }) {
  ...
  return (
    <button
      onClick={onSelect}
      className="w-full flex items-center justify-between gap-3 p-4 hover:bg-slate-50 dark:hover:bg-slate-50 dark:bg-white/5 transition text-left"
    >
