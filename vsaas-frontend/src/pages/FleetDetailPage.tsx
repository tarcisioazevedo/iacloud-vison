    ...
    <div className="rounded-xl border border-slate-200 dark:border-white/8 bg-white dark:bg-space-900 p-4 space-y-4">
        ...
        <div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead>
                <tr className="text-left border-b border-slate-200 dark:border-white/5">
              ...
              <tbody>
                {Object.entries(frigateCams).map(([cam, m]) => (
                  <tr key={cam} className="border-b border-slate-200 dark:border-white/5 last:border-0">
                    <td className="py-1.5 pr-3 text-slate-200">{cam}</td>
                    <td className="py-1.5 pr-3 text-right font-mono text-slate-600 dark:text-slate-300">{m.usageGB?.toFixed(2) ?? '—'}</td>
      ...
      {(storage.retentionDays || storage.recordingsGB || storage.thumbnailsGB) && (
        <div className="flex flex-wrap gap-3 text-[11px] text-slate-500">
          {storage.retentionDays != null && <span>Retenção: <span className="text-slate-600 dark:text-slate-300">{storage.retentionDays}d</span></span>}
          {storage.recordingsGB != null && <span>Recordings: <span className="text-slate-600 dark:text-slate-300">{storage.recordingsGB.toFixed(1)} GB</span></span>}
          {storage.exportsGB != null && <span>Exports: <span className="text-slate-600 dark:text-slate-300">{storage.exportsGB.toFixed(1)} GB</span></span>}
          {storage.thumbnailsGB != null && <span>Thumbs: <span className="text-slate-600 dark:text-slate-300">{storage.thumbnailsGB.toFixed(1)} GB</span></span>}
        </div>
      )}
    </div>
  )
              ...
              <button
                ...
                className={`text-left px-2.5 py-1.5 rounded-lg border transition ${
                  type === spec.type
                    ...
                    : 'border-slate-200 dark:border-white/8 hover:border-indigo-300 dark:hover:border-indigo-500/30 hover:bg-slate-50 dark:hover:bg-slate-50 dark:bg-white/5'
                }`}
              >
