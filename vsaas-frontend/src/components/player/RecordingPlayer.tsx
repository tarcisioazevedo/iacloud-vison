...
export function RecordingPlayer({
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        'relative flex flex-col w-full bg-black rounded-xl overflow-hidden border border-slate-200 dark:border-white/10',
        className,
      )}
    >
      {/* Vídeo */}
      ...
      {/* Controles */}
      <div className="flex flex-col gap-2 px-3 py-2 bg-white dark:bg-slate-900/95 dark:bg-slate-950/95 border-t border-slate-200 dark:border-white/5">
        {/* Linha 1: timeline */}
        <RecordingTimeline
          cameraId={cameraId}
        ...
        <div className="flex items-center gap-1 flex-wrap">
          ...
          {sortedBookmarks.length > 0 && (
            <button
              ...
              className="p-1.5 rounded-md text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:bg-white/10 hover:text-slate-900 dark:text-white"
            >
          ...
          <div className="flex items-center">
            <button
              onClick={() => skipBy(-skip)}
              title={`Voltar ${skip}s`}
              className="px-2 py-1.5 rounded-l-md text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:bg-white/10 hover:text-slate-900 dark:text-white text-[11px] font-mono flex items-center gap-1"
            >
            ...
            <div className="relative">
              <button
                ...
                className="px-1.5 py-1.5 rounded-r-md text-slate-400 hover:bg-slate-100 dark:bg-white/10 hover:text-slate-900 dark:text-white border-l border-slate-200 dark:border-white/10"
              >
                <ChevronRight className={cn('w-3 h-3 transition-transform', showSkipMenu && 'rotate-90')} />
              </button>
              {showSkipMenu && (
                <div className="absolute bottom-full mb-1 left-0 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-md shadow-xl z-30 min-w-[80px]">
                  {SKIP_OPTIONS_SEC.map(s => (
                    <button
                      key={s}
                      onClick={() => { setSkip(s); setShowSkipMenu(false) }}
                      className={cn(
                        'block w-full text-left px-2 py-1 text-[11px] font-mono hover:bg-slate-100 dark:bg-white/10',
                        s === skip ? 'text-cyan-300' : 'text-slate-600 dark:text-slate-300',
                      )}
                    >
                      {s < 60 ? `${s}s` : `${s / 60}min`}
                    </button>
          ...
          <button
            ...
            className="px-2 py-1.5 rounded-md text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:bg-white/10 hover:text-slate-900 dark:text-white text-[11px] font-mono flex items-center gap-1"
          >
          ...
          <div className="ml-auto flex items-center gap-1">
            ...
            <div className="relative">
              ...
              {showSpeedMenu && (
                <div className="absolute bottom-full mb-1 right-0 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-md shadow-xl z-30 min-w-[70px]">
                  {SPEEDS.map(s => (
                    <button
                      key={s}
                      ...
                      className={cn(
                        'block w-full text-left px-2 py-1 text-[11px] font-mono hover:bg-slate-100 dark:bg-white/10',
                        s === speed ? 'text-cyan-300' : 'text-slate-600 dark:text-slate-300',
                      )}
                    >
                      {s}×
                    </button>
            ...
            {audioTracks.length > 1 && (
              <div className="relative">
                <button
                  ...
                  className="px-2 py-1.5 rounded-md text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:bg-white/10 hover:text-slate-900 dark:text-white text-[10px] font-mono"
                  title="Trilha de áudio"
                >
                  AUD
                </button>
                {showAudioMenu && (
                  <div className="absolute bottom-full mb-1 right-0 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-md shadow-xl z-30 min-w-[140px]">
                    {audioTracks.map(t => (
                      <button
                        key={t.id}
                        onClick={() => setAudioTrack(t.id)}
                        className={cn(
                          'block w-full text-left px-2 py-1 text-[11px] hover:bg-slate-100 dark:bg-white/10',
                          t.id === activeAudio ? 'text-cyan-300' : 'text-slate-600 dark:text-slate-300',
                        )}
                      >
                        {t.label}
                      </button>
            ...
            <button
              onClick={cycleFit}
              title={`Modo: ${fitMode}`}
              className="px-2 py-1.5 rounded-md text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:bg-white/10 hover:text-slate-900 dark:text-white text-[10px] font-mono uppercase flex items-center gap-1"
            >
