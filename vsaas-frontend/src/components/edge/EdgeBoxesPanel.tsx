        ...
        <div className="flex items-center justify-between gap-3 mb-4 flex-wrap">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}
              className="px-2 py-1.5 rounded-lg bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 text-xs text-slate-900 dark:text-white">
            ...
            <button onClick={() => mutate()} title="Atualizar"
              className="p-1.5 rounded-lg bg-slate-50 dark:bg-white/5 hover:bg-slate-100 dark:bg-white/10 border border-slate-200 dark:border-white/10 text-slate-400">
...
function EdgeBoxCard({ box, onClick, onViewKey, onRotate, onDecommission, onSuspend, onResume }: {
  ...
  return (
    <div className="p-3 rounded-lg bg-white/[0.02] border border-slate-200 dark:border-white/10 hover:border-white/20 transition cursor-pointer group"
      onClick={onClick}>
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            <p className="text-sm font-medium text-slate-900 dark:text-white truncate">{box.name}</p>
      ...
      {/* Ações */}
      <div className="mt-2 pt-2 border-t border-slate-200 dark:border-white/5 grid grid-cols-2 gap-1" onClick={e => e.stopPropagation()}>
      ...
      <motion.form initial={{ y: 12 }} animate={{ y: 0 }} onClick={e => e.stopPropagation()} onSubmit={submit}
        ...
        <div>
          ...
          <select value={form.siteId} onChange={e => set('siteId', e.target.value)} required
            className="w-full px-3 py-2 rounded-lg bg-slate-100 dark:bg-space-900 border border-slate-200 dark:border-white/10 text-xs text-slate-900 dark:text-white [&>option]:bg-white [&>option]:text-slate-900 dark:[&>option]:bg-space-900 dark:[&>option]:text-slate-900 dark:text-white">
        ...
        <div className="flex gap-2">
          <button type="button" onClick={onClose} className="flex-1 px-3 py-2 rounded-lg bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 text-xs text-slate-400">Cancelar</button>
      ...
      <motion.div initial={{ y: 12 }} animate={{ y: 0 }} onClick={e => e.stopPropagation()}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <div className="p-2.5 rounded-lg bg-white dark:bg-slate-900 dark:bg-black/60 border border-slate-300 dark:border-slate-700">
        ...
        <div className="flex gap-2 pt-2 border-t border-slate-200 dark:border-white/5">
          <button onClick={() => {
            ...
            className="flex-1 px-3 py-2 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-white/5 dark:hover:bg-slate-100 dark:bg-white/10 border border-slate-200 dark:border-white/10 text-slate-700 dark:text-slate-300 text-xs font-bold">
            Baixar .txt
          </button>
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ x: 400 }} animate={{ x: 0 }} exit={{ x: 400 }}
        onClick={e => e.stopPropagation()}
        className="w-full max-w-md bg-white dark:bg-space-900 border-l border-slate-200 dark:border-white/10 p-5 overflow-y-auto space-y-4">
        <div className="flex items-start justify-between">
          <div>
            <h2 className="text-lg font-bold text-slate-900 dark:text-white">{box.name}</h2>
            <p className="text-xs text-slate-500 font-mono">{box.serialNumber}</p>
          </div>
          <button onClick={onClose} className="p-1 text-slate-500 hover:text-slate-900 dark:text-white">
            <X className="w-5 h-5" />
          </button>
        </div>
...
function SkeletonGrid() {
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
        {[0,1,2,3,4,5].map(i => <div key={i} className="h-16 rounded-lg bg-slate-50 dark:bg-white/5 animate-pulse" />)}
      </div>
      <GlassCard className="p-4">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {[0,1,2,3,4,5].map(i => <div key={i} className="h-32 rounded-lg bg-slate-50 dark:bg-white/5 animate-pulse" />)}
        </div>
      </GlassCard>
    </div>
  )
