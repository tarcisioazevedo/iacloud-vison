    ...
    <div className="space-y-4">
      ...
      {mode === 'standalone' && (
        <GlassCard className="p-5 bg-gradient-to-br from-violet-500/10 to-transparent border-violet-500/20">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-white">Logs Explorer</h1>
      ...
      <GlassCard className="p-3">
          ...
          {PERIOD_PRESETS.map(p => (
            <button key={p.label} onClick={() => { setPeriodHours(p.hours); setPage(1) }}
              className={cn('px-2 py-0.5 rounded text-[10px] font-bold border transition',
                periodHours === p.hours ? 'bg-violet-500/30 text-violet-200 border-violet-500/50' : 'text-slate-400 border-white/10 hover:border-white/20')}>
              {p.label}
            </button>
          ))}
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button onClick={() => setShowFilters(s => !s)}
            className="ml-auto px-2 py-1 rounded text-[10px] bg-white/5 hover:bg-white/10 text-slate-400 flex items-center gap-1">
          ...
          <div className="mt-3 pt-3 border-t border-white/5 grid grid-cols-1 sm:grid-cols-3 gap-2">
            ...
            <div className="relative col-span-2">
              ...
              <input value={search} onChange={e => { setSearch(e.target.value); setPage(1) }}
                ...
                className="w-full pl-7 pr-2 py-1.5 rounded bg-white/5 border border-white/10 text-xs text-white" />
            </div>
          </div>
        )}
      </GlassCard>
          ...
          <div className="flex flex-wrap gap-2">
            {agg.topActors.map(a => (
              <button key={a.email} onClick={() => { setActorEmail(a.email); setPage(1) }}
                className="px-2 py-1 rounded-full text-[10px] bg-white/5 hover:bg-violet-500/20 hover:text-violet-300 text-slate-300 border border-white/10">
      ...
      <GlassCard className="p-3">
        <div className="flex items-center justify-between mb-3">
          <p className="text-sm font-semibold text-white">Eventos ({data?.total?.toLocaleString('pt-BR') ?? 0})</p>
        ...
        {error ? (
          ...
          <div className="space-y-1">
            {[0,1,2,3,4].map(i => <div key={i} className="h-8 bg-white/5 rounded animate-pulse" />)}
          </div>
        ) : logs.length === 0 ? (
            ...
            {data && data.pages > 1 && (
              <div className="flex items-center justify-center gap-2 mt-3 pt-3 border-t border-white/5">
                <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
                  className="px-3 py-1 rounded bg-white/5 hover:bg-white/10 text-xs text-slate-400 disabled:opacity-30">
                  Anterior
                </button>
          ...
          <td colSpan={8} className="px-3 py-2">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-[10px]">
              <div>
                ...
                <code className="text-slate-300 font-mono">{log.resource}:{log.resourceId}</code>
              </div>
              <div>
                <p className="uppercase text-slate-500 mb-1">User-Agent</p>
                <code className="text-slate-300 font-mono break-all">{log.userAgent ?? '—'}</code>
              </div>
            </div>
            <p className="text-[10px] uppercase text-slate-500 mt-2 mb-1">Metadata</p>
            <pre className="text-[10px] text-slate-300 bg-black/40 p-2 rounded overflow-x-auto font-mono max-h-40">
