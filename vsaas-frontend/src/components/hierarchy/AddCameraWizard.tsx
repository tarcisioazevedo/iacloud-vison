...
export function AddCameraWizard({ open, onClose, siteId: siteIdProp, edgeNodeId: initialEdgeNodeId, onCreated }: AddCameraWizardProps) {
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-3xl bg-white dark:bg-slate-900 border border-rose-500/30 rounded-2xl shadow-2xl shadow-rose-500/10 overflow-hidden my-8"
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="px-6 py-5 border-b border-slate-200 dark:border-slate-800 bg-gradient-to-r from-rose-500/5 to-violet-500/5">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold text-slate-900 dark:text-white">Adicionar câmera</h2>
                <p className="text-xs text-slate-400">Configure como esta câmera vai se conectar</p>
              </div>
            </div>
            <button onClick={handleClose} className="p-2 rounded-lg hover:bg-slate-100 dark:bg-slate-800 text-slate-400 hover:text-slate-900 dark:text-white">
              <X className="w-4 h-4" />
            </button>
          </div>
          {/* Step indicator */}
        ...
        {step !== 4 && (
          <div className="px-6 py-4 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/50 flex items-center justify-between">
            <button
              ...
              className="px-4 py-2 rounded-lg text-slate-400 hover:text-slate-900 dark:text-white text-sm transition flex items-center gap-2"
            >
...
function StepDot({ n, active, done, label }: { n: number; active: boolean; done: boolean; label: string }) {
  return (
    <span className="flex items-center gap-1.5">
      ...
      <span className={cn(active ? 'text-slate-900 dark:text-white font-bold' : 'text-slate-500')}>{label}</span>
    </span>
  )
}
...
function Step1ModeSelect({
  ...
  return (
    <>
      ...
      {needsSiteSelector && (
        <div className="mb-5 p-4 rounded-xl bg-rose-500/10 border-2 border-rose-500/40">
          ...
          <select
            ...
            className="w-full px-3 py-2 text-sm rounded-lg bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white focus:border-rose-400 focus:outline-none"
          >
      ...
      <div className="text-sm text-slate-600 dark:text-slate-300 mb-5">
        <strong className="text-slate-900 dark:text-white">Como esta câmera vai se conectar?</strong>
              ...
              <button
                ...
                className={cn(
                  ...
                  protocol === p
                    ...
                    : 'border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-400 hover:border-violet-500/50',
                )}
                title={
      ...
      {/* Outras opções */}
      <div className="mt-5 p-4 rounded-xl bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800">
        <div className="text-xs font-bold text-slate-600 dark:text-slate-300 mb-2">⚡ Outras opções (em breve)</div>
        <div className="flex flex-wrap gap-2 text-xs text-slate-500">
          <span className="px-3 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-800/50 border border-slate-300 dark:border-slate-700/50">📋 Importar lote (CSV)</span>
          <span className="px-3 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-800/50 border border-slate-300 dark:border-slate-700/50">🔍 Auto-discovery ONVIF</span>
          <span className="px-3 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-800/50 border border-slate-300 dark:border-slate-700/50">📡 QR code (box-installer)</span>
        </div>
      </div>
    </>
  )
    ...
    <div className="space-y-4">
      <div>
        ...
        <input
          ...
          className="w-full px-3 py-2 rounded-lg bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-sm text-slate-900 dark:text-white placeholder:text-slate-500 focus:border-rose-500/50 focus:outline-none"
        />
      </div>
        ...
        <>
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 rounded-lg bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-sm text-slate-900 dark:text-white font-mono placeholder:text-slate-500 focus:border-rose-500/50 focus:outline-none"
            />
          </div>
  ...
  return (
    <div className="space-y-4">
      <div className="text-sm text-slate-600 dark:text-slate-300">
        <strong className="text-slate-900 dark:text-white">Confira os dados antes de criar:</strong>
      </div>
...
function Row({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="flex items-center justify-between gap-3 border-b border-slate-200 dark:border-slate-800/50 pb-2 last:border-0">
      <dt className="text-xs uppercase tracking-wider text-slate-500 font-medium">{label}</dt>
      <dd className="text-sm text-slate-900 dark:text-white text-right">{children}</dd>
    </div>
  )
}
  ...
  return (
    <div className="mb-3">
      ...
      <div className="flex items-center gap-2 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-lg px-3 py-2">
        ...
        <button onClick={copy} className="shrink-0 text-slate-400 hover:text-slate-900 dark:text-white transition">
  ...
  return (
    <div className="py-4">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h3 className="text-lg font-bold text-slate-900 dark:text-white">Câmera criada! 🎉</h3>
        ...
        <div className="mb-4">
          <div className="p-3 rounded-lg bg-violet-500/10 border border-violet-500/30 mb-3">
            ...
            <p className="text-[11px] text-slate-400">
              ...
              o <code className="bg-slate-100 dark:bg-slate-800 px-1 rounded">streamid</code> e a
              <code className="bg-slate-100 dark:bg-slate-800 px-1 rounded">latency</code> já vão embutidos.
      ...
      <div className="flex items-center justify-end gap-2">
        <button
          onClick={onClose}
          className="px-4 py-2 rounded-lg bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 hover:border-slate-600 text-sm text-slate-900 dark:text-white transition"
        >
          Fechar
        </button>
        {cameraId && (
