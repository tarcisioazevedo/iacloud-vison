          ...
          {pending ? (
            <div className="p-2.5 rounded-lg bg-amber-500/10 border border-amber-500/30 text-xs">
              ...
              <p className="text-slate-400">
                {pending.fromPlano?.name ?? 'plano atual'} → <strong className="text-white">{pending.toPlano.name}</strong>
              </p>
...
function UpgradeModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="w-full max-w-3xl max-h-[90vh] overflow-y-auto bg-slate-900 border border-cyan-500/30 rounded-2xl shadow-2xl">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
                {ps.sort((a, b) => a.retainDays - b.retainDays).map(p => {
                  ...
                  return (
                    ...
                    >
                      <p className="text-sm font-bold text-white">{p.retainDays}d</p>
