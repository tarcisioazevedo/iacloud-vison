...
| ◐ | **Pipeline colapsa às 03:00 UTC** (00:00 BRT) e tenta restart inútil a cada ~2h | 4 buracos no dia 2026-05-08: 5h39, 2h, 1h34, 2h. Bursts pós-blackout duram só 30-90s e param. | **Cloud Direct corrigido 2026-05-19**: `recording.service.ts` agora cria dirs de hoje E amanhã UTC no spawn do ffmpeg (evita crash de meia-noite). Box: ainda precisa investigar logs go2rtc/Frigate ao redor de 03:00 UTC. | Câmera grava 24/7 sem intervenção por ≥48h consecutivas |
...
| ☑ | **`RECORDING_DELETE_LOCAL_AFTER_S3=false` (padrão)** — tmpfs de 4 GB enche em <1h com câmeras reais | `RECORDING_DELETE_LOCAL_AFTER_S3: "true"` confirmado em `docker-stack.yml:411` — **já coberto em produção** | Arquivo `.ts` desaparece do tmpfs após upload confirmado no R2 |
...
| ☑ | **`RECORDING_DELETE_LOCAL_AFTER_S3=true`** | Confirmado `"true"` em `docker-stack.yml:411` — **já ativo em prod** (a referência a `secrets/s3.env` era obsoleta) | Disco da VPS não estoura com câmeras reais |
...
| ☑ | **R2_QUEUE_ID configurado em prod** | Confirmado `docker-stack.yml:425` = `2e3c8f5641894d5781b1ed4d08224342` — **já ativo** | `r2-event-consumer.service.ts` processa eventos de upload |
| ☑ | **Custom Domain por integrador (white-label)** | **Entregue 2026-05-19**: CF Worker KV auto-provision, tenant-context fallback por Host header, `/internal/domain-check`, Caddyfile on-demand TLS, SMTP por integrador | Integrador configura `monitor.meuisp.com.br`, cert HTTPS automático, e-mail com domínio próprio |
| ☐ | **Health summary cron + dashboard** | Sprint 5 em curso | novo `health-summary-cron.service.ts` |
| ☑ | **`RECORDING_DELETE_LOCAL_AFTER_S3=true` em prod** | Confirmado `docker-stack.yml:411` — **já ativo** (referência a `secrets/s3.env` era obsoleta) | Disco da VPS não estoura |
...
| 2026-05-19 | **White-label completo + fixes de pipeline + auditoria de checklist** — (1) SMTP por integrador: schema `IntegradorSmtpConfig`, rotas `/me/integrador/smtp`, UI `MeWhitelabelPage` com test-email inline. (2) CF Worker `icv-tenant-router` deployado com KV auto-provision/deprovision ao verificar/deletar custom domain. (3) `tenant-context.ts` com fallback por Host header para domínios externos (sem CF Worker). (4) `/internal/domain-check` endpoint para Caddy on-demand TLS (localhost-only, rejeita reservados, só ACTIVE). (5) Caddyfile atualizado: bloco `:443` com `on_demand` TLS — Caddy emite cert Let's Encrypt automático para qualquer domínio verificado. (6) Cloud Direct recorder: fix midnight crash (cria dirs hoje+amanhã UTC) + `-reset_timestamps 1` em ambos os recorders (HLS PTS spec). (7) Auditoria corrigiu 4 ☐ que já estavam resolvidos: `RECORDING_DELETE_LOCAL_AFTER_S3`, `R2_QUEUE_ID`, Custom Domain white-label. Token CF substituído por "VSAAS" com duas políticas corretas (KV/Workers Conta inteira + DNS zona). |
